...
internal sealed class ProxyCacheManager : IDisposable
{
    ...
    internal ProxyCacheEntry? TryGetProxy(string originalPath, float scale)
    {
        ...
        if (_videoCache is not null &&
            ProxyForgeSettings.Default.EnableVideoCache &&
    ...
    internal void StartProxyGeneration(string originalPath, float scale, ProxyForgeSettings settings)
    {
    ...
    private async Task RunGenerationAsync(CacheKey key, string originalPath, float scale, ProxyForgeSettings settings, CancellationTokenSource cts)
    {
        ...
        catch (Exception ex)
        {
            delayMs = 8000;
            var msg = ex.Message;
            Debug.WriteLine(string.Concat("[ProxyForge] Proxy generation failed for ", originalPath, ": ", ex));
            DispatchInvoke(() =>
            {
