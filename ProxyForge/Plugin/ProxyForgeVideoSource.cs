...
internal sealed class ProxyForgeVideoSource : IVideoFileSource
{
    ...
    internal ProxyForgeVideoSource(IVideoFileSource inner, IGraphicsDevicesAndContext devices)
    {
        _inner = inner;
    ...
    internal ProxyForgeVideoSource(IVideoFileSource inner, IGraphicsDevicesAndContext devices, Func<IVideoFileSource?> tryUpgradeSource)
    {
        _inner = inner;
        _switchEffect = new AffineTransform2D(devices.DeviceContext);
        _switchEffect.SetInput(0, inner.Output, true);
