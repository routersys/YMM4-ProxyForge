...
public sealed class ProxyEncoderTests
{
    ...
    public async Task EncodesAProxyAtTheRequestedScale()
    {
        ...
        var result = await encoder.EncodeAsync(workspace.Request(50), null, TestContext.Current.CancellationToken);

        Assert.Equal(new ProxyGeometry(640, 360), result.Geometry);
        Assert.Equal(new FrameRate(30, 1), result.FrameRate);
        Assert.Equal(30, result.FrameCount);
    ...
    public async Task MeasuresTheBoundsAsDirect2DRasterisesThem()
    {
        ...
        Assert.Equal(new DisplayBounds(-641f, -361f, 1282f, 722f), result.Display);
        Assert.Equal(new ProxyGeometry(640, 360), result.Geometry);
    }
    ...
    public async Task RendersEveryFrameOfTheSourceOnce()
    {
        ...
        await encoder.EncodeAsync(workspace.Request(50), null, TestContext.Current.CancellationToken);
    ...
    public async Task ReportsProgressInAscendingSteps()
    {
        ...
        await encoder.EncodeAsync(workspace.Request(50), new SynchronousProgress(reports.Add), TestContext.Current.CancellationToken);
    ...
    public async Task PreservesTheNtscFrameRateExactly()
    {
        ...
        Assert.Equal(new FrameRate(30000, 1001), result.FrameRate);
    ...
    public async Task AlignsAnOddSizedSourceSymmetrically(TestCentering centering)
    {
        ...
        Assert.Equal(new ProxyGeometry(320, 180), result.Geometry);
    ...
    public async Task APortraitSourceProducesAPortraitProxy()
    {
        ...
        Assert.Equal(new ProxyGeometry(540, 960), result.Geometry);
    }
    ...
    public async Task ATransparentSourceIsRefusedAndLeavesNoFile()
    {
        ...
        var exception = await Assert.ThrowsAsync<ProxyEncodeException>(() => encoder.EncodeAsync(workspace.Request(50), null, TestContext.Current.CancellationToken));
    ...
    public async Task AnAlreadyCancelledTokenLeavesNoFile()
    {
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => encoder.EncodeAsync(workspace.Request(50), null, cancellation.Token));
        Assert.False(File.Exists(workspace.OutputPath));
    }
    ...
    public async Task CancellationDuringTheEncodeLeavesNoFile()
    {
        ...
        await Assert.ThrowsAnyAsync<OperationCanceledException>(() => encoder.EncodeAsync(workspace.Request(50), progress, cancellation.Token));
        Assert.False(File.Exists(workspace.OutputPath));
    }
    ...
    public async Task ARelativeSourcePathIsRejected()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(() => encoder.EncodeAsync(workspace.Request(50) with { SourcePath = "relative.mp4" }, null, TestContext.Current.CancellationToken));
    }
    ...
    public async Task AnOutputOutsideTheWorkingDirectoryIsRejected()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(() => encoder.EncodeAsync(
            workspace.Request(50) with { OutputPath = Path.Combine(Path.GetTempPath(), "escaped.mp4") },
            null,
    ...
    public async Task AFullHdSourceOfRealisticLengthEncodes()
    {
        ...
        Assert.Equal(new ProxyGeometry(960, 540), result.Geometry);
    ...
    sealed class Workspace : IDisposable
    {
        ...
        public ProxyEncodeRequest Request(int scale) => new(SourcePath, OutputPath, Path, scale, 50, 30, false);
            ...
            Func<int, int, (byte Blue, byte Green, byte Red, byte Alpha)>? pixelOf = null)
        {
            ...
            {
                var source = new TestVideoSource(devices, width, height, frameRateNumerator, frameRateDenominator, frameCount, pixel, centering);
                Source = source;
                return source;
            });
        }
